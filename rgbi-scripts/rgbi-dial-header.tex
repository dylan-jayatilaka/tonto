...
% A big canvas, on purpose.
%
% Tonto lays the dials out four to a row (put_dial_table_do_H in roby.foo).
% Four of them need about 520pt, and `article`'s default \textwidth is about
% 345pt -- so the fourth column always hung off the right-hand page edge, and
% pdfcrop, which crops to the MediaBox, simply cut it off. The dial was not
% mis-drawn; it was never on the page.
%
% The final PDF is cropped to its content, so the paper size does not survive
% into the result: it only has to be large enough that nothing falls off. Hence
% a2 landscape with thin margins. Costs nothing and clips nothing.
%
% This makes the page fit the table. The other direction -- making the table
% fit the page, so the number of columns adapts to the dial size instead of
% being hard-coded to four in three places -- is in TASKS_AND_HISTORY.md.
\usepackage[a2paper,landscape,margin=10mm]{geometry}
