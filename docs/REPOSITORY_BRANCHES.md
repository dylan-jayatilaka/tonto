...
| `archive/nn-har` | **Leave it.** Per Dylan (2026-08-15) it is really an unfinished least-squares rewrite, and the code agrees. Its one apparently-salvageable item is superseded: Dylan's own final commit `fd956388` ("Corrected fragHAR to work with DFT") changes a string-prefix test, `scfdata.scf_kind(1:7)=="fraghar"` to `(1:8)=="fragment"`, because with DFT the kind does not begin "fraghar". Master no longer makes that test at all -- the 2026 fragHAR repair (`d840e322`) replaced the mechanism with a proper predicate, `CRYSTAL:do_fragHAR` (`crystal.foo:3073`). The NN machinery itself is already present on master and commented out (`make_NN_capped_groups`, `molecule.base.foo:1753`). The branch also carries an acknowledged gap in its own comment -- "NOTE ERRORS SHOULD ALSO BE SET TO ZERO HERE, BUT I AM LAZY MD 10/02/2023" -- and the author's commit message records the results as underwhelming. 244 insertions, old `:::` dialect. |
