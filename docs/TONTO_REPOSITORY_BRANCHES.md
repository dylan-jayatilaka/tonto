...
`TASKS_AND_HISTORY.md` as an open feature with a scientific obstacle, not as an unported branch.
...
| `archive/bond-energy` | **will not be ported** — decided, not postponed. Stays on its tag; see `TASKS_AND_HISTORY.md` |
...
and needs a `types.foo` change. `TASKS_AND_HISTORY.md` carries the full inventory.
...
`develop`; and the `MAT3{VEC_{INT}}` spelling, now `MAT3{EVEC{INT}}`) are in `TASKS_AND_HISTORY.md`
...
fetch with `curl`; the recipe is in `TASKS_AND_HISTORY.md` under the in-core CCSD search.
...
| `archive/kanghyun` | Nothing to port. The `oisn't` → `isn't` typo was **already fixed on `master`** independently. Commenting out `stdout.flush` in `object.foo` was a workaround for stray blank lines in the keyword echo; the real cause — `TEXTFILE:flush` emitting the margin twice — was root-caused and fixed on 2026-08-03 (see `TASKS_AND_HISTORY.md`), so the workaround is obsolete and treats the symptom. Only the two-line CIF/job-name echo is live, and it was judged not worth the output change. |
