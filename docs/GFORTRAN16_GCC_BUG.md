...
**Not filed — it needs a Bugzilla login, and there is no account.** Everything
below is ready to paste into the "File a new bug" form for gcc/fortran once
...
catch something (44744, 82243, 111339, 34740, 39772, 81095) — or an ICE (83953).
None is wrong code *generated by* the check. Worth repeating over resolved bugs
and `"16 Regression"` before filing; that search was cut short by sourceware
rate-limiting, which the site says logging in relieves.

Ruled out as the same bug: commit **r16-2249** *"fortran: Amend descriptor bounds
init if unallocated"* (PR fortran/108889) looks close by title but is about
suppressing uninitialised warnings in reallocation-on-assignment, and touches
